...
def audio_classification_predict(args):
    ...
    model_1 = build_model((SAMPLING_RATE // 2, 1), 4)
...
########################################################################################################################################################
    ...
    # model = build_model((SAMPLING_RATE, 1), 3)
    model_2 = build_model((SAMPLING_RATE // 2, 1), 4)
...
########################################################################################################################################################
    ...
    for count, file in enumerate(test_files_path):
        ...
        for count_many in range(10, (len(data)//SAMPLING_RATE)):
            ...
            y_pred_pro_1 = model_1.predict(fft)
            # Take random samples
            y_pred_1 = np.argmax(y_pred_pro_1, axis=-1) # 0 :female / 1 : male / 2 : noise / 3 : children
            
            y_pred_pro_2 = model_2.predict(fft)
            # Take random samples
            y_pred_2 = np.argmax(y_pred_pro_2, axis=-1) # 0 :female / 1 : male / 2 : noise / 3 : children
            #print(round(np.max(y_pred_pro), 3))
            #print(np.max(y_pred_pro))
            #print(y_pred_pro)

            if y_pred_1[0] == 0 and y_pred_2[0] == 0: # female
                #print("{}s ~ {}s".f2"{}s ~ {ormat(start/SAMPLING_RATE, end/SAMPLING_RATE))
            ...
            elif y_pred_1[0] == 1 and y_pred_2[0] == 1: # male
            ...
            elif y_pred_1[0] == 3 and y_pred_2[0] == 3: # children
                #print("{}s ~ {}s".format(start/SAMPLING_RATE, end/SAMPLING_RATE))
